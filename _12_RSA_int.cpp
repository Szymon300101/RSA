...
void Alice()
{
    ...
    string t;
    long long p1=rGenPrime(pMIN,pMAX,randC);                //1 part of n
    long long p2=rGenPrime(p1-pSENS,p1+pSENS,randC);        //2 part of n
        if(p1==p2) p2=rGenPrime(p2-pSENS,p2+pSENS,randC);
    long long n=p1*p2;  //making n key
    long long fiN=(p1-1)*(p2-1);    //Euler's function (fi) of n
    long long e=e0;         //making e key
        while(fiN%e==0) e+=2;
    long long k=1;              //making d key (Private)
    long long d=(k*fiN+1)/e;
    while((long long)(k*fiN+1)%e!=0)
    {
        k++;
        d=(k*fiN+1)/e;
        if(k>50)
    ...
    //cout<<"d= "<<d<<endl;
    cout<<"Insert c: "; cin>>t;
    int mult=0;                     //my original algorithm witch let user to send
    if(t.find('.',0)!=-1)           //messages bigger than n.
    {
        int f=t.find('.',0);
        string in;
        for(int i=0;i<f;i++)
...
void Bob()
{
    ...
    int cnt=0;              //sending part of my algorithm.
    while(m>n)
    {
        m-=n;
        cnt++;
